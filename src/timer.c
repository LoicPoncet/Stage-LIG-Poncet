...
inline static double differenceTimer(const struct timeval *tOrigin,const struct timeval *tEnd){
	return (tEnd->tv_sec - tOrigin->tv_sec) + (tEnd->tv_usec - tOrigin->tv_usec)/1000000.0;
}
